...
A hardened CLI agent with layered tool security:
...
def run_agent_turn(messages: list[dict[str, Any]], *, yolo: bool) -> str:
    while True:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Lesson 10 secure agent")
    parser.add_argument(
        "--yolo",
        action="store_true",
...
def main() -> None:
    ...
    print("Secure Agent (type 'quit' to exit)")
    print("-" * 40)
    print("Available tools:", ", ".join(sorted(TOOL_HANDLERS.keys())))
    print(f"Workspace root: {WORKSPACE_ROOT}")
    print(f"HTTP allowlist: {', '.join(sorted(ALLOWED_HTTP_DOMAINS))}")
    print(f"Shell allowlist: {', '.join(sorted(ALLOWED_SHELL_COMMANDS))}")
    print(f"Confirmation bypass (--yolo): {args.yolo}")
    ...
    while True:
        ...
        run_agent_turn(messages, yolo=args.yolo)
